...
// MARK: - Theme (colours taken from calibrated competition plates)

enum Theme {
    static let accent = Color(red: 0.11, green: 0.31, blue: 0.61)       // 20 kg blue
    static let plateRed = Color(red: 0.82, green: 0.15, blue: 0.22)     // 25 kg
    static let plateYellow = Color(red: 0.93, green: 0.72, blue: 0.20)  // 15 kg
    static let plateGreen = Color(red: 0.18, green: 0.55, blue: 0.34)   // 10 kg
    static let noData = Color(.systemGray4)

    /// Colour per Strength Score level.
    static func color(_ level: StrengthLevel?) -> Color {
        guard let level else { return noData }
        switch level {
        case .beginner: return Color(.systemGray)
        case .novice: return plateGreen
        case .intermediate: return plateYellow
        case .advanced: return accent
        case .elite: return plateRed
        case .worldClass: return Color(red: 0.55, green: 0.25, blue: 0.75)
        }
    }
    ...
    static func tier(_ percentile: Double?) -> (name: String, color: Color) {
        ...
        switch p {
        case ..<25: return ("Developing", plateGreen)
        case ..<50: return ("Intermediate", plateYellow)
        case ..<75: return ("Advanced", accent)
        case ..<90: return ("Elite", plateRed)
        default: return ("Top 10%", .primary)
        }
    }
}
