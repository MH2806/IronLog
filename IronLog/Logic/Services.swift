...
enum HealthSync {
    ...
    static func requestAccess() async throws {
        ...
        try await store.requestAuthorization(toShare: [HKObjectType.workoutType()],
                                             read: [HKQuantityType(.bodyMass), HKQuantityType(.restingHeartRate),
                                                    HKCategoryType(.sleepAnalysis)])
    }

    /// Hours asleep in the 24 h before now, or nil if unavailable/no permission.
    static func lastNightSleepHours() async -> Double? {
        ...
        return await withCheckedContinuation { cont in
            let q = HKSampleQuery(sampleType: HKCategoryType(.sleepAnalysis), predicate: pred, limit: HKObjectQueryNoLimit,
    ...
    /// Latest resting heart rate (bpm) in the last 3 days.
    static func restingHeartRate() async -> Double? {
        guard available else { return nil }
        let pred = HKQuery.predicateForSamples(withStart: Date().addingTimeInterval(-3 * 86400), end: .now)
        return await withCheckedContinuation { cont in
            let sort = NSSortDescriptor(key: HKSampleSortIdentifierEndDate, ascending: false)
            let q = HKSampleQuery(sampleType: HKQuantityType(.restingHeartRate), predicate: pred, limit: 1,
                                  sortDescriptors: [sort]) { _, samples, _ in
                guard let s = samples?.first as? HKQuantitySample else { return cont.resume(returning: nil) }
                cont.resume(returning: s.quantity.doubleValue(for: HKUnit.count().unitDivided(by: .minute())))
            }
            store.execute(q)
        }
    }
