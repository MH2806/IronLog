...
enum ToolCatalog {
    static let all: [ToolItem] = [
        ToolItem(title: "Strength level", icon: "trophy.fill", color: Palette.green, group: "Strength") { AnyView(StrengthLevelTool()) },
        ToolItem(title: "1RM", icon: "scalemass.fill", color: Palette.blue, group: "Strength") { AnyView(OneRMTool()) },
        ToolItem(title: "RPE", icon: "gauge.with.dots.needle.67percent", color: Palette.blue, group: "Strength") { AnyView(RPETool()) },
        ToolItem(title: "% chart", icon: "percent", color: Palette.blue, group: "Strength") { AnyView(PercentageTool()) },
        ToolItem(title: "Warm-up", icon: "flame.fill", color: Palette.orange, group: "Strength") { AnyView(WarmupTool()) },
        ToolItem(title: "Plates", icon: "circle.circle.fill", color: Palette.red, group: "Strength") { AnyView(PlateTool()) },
        ToolItem(title: "Volume", icon: "chart.bar.xaxis", color: Palette.purple, group: "Strength") { AnyView(VolumeTool()) },
        ToolItem(title: "DOTS", icon: "d.circle.fill", color: Palette.yellow, group: "Strength") { AnyView(DotsTool()) },
        ToolItem(title: "Wilks", icon: "w.circle.fill", color: Palette.yellow, group: "Strength") { AnyView(WilksTool()) },
        ToolItem(title: "Body fat", icon: "percent", color: Palette.orange, group: "Body & nutrition") { AnyView(BodyFatTool()) },
        ToolItem(title: "Lean mass", icon: "figure.stand", color: Palette.green, group: "Body & nutrition") { AnyView(LBMTool()) },
        ToolItem(title: "FFMI", icon: "figure.arms.open", color: Palette.green, group: "Body & nutrition") { AnyView(FFMITool()) },
        ToolItem(title: "Ideal weight", icon: "ruler.fill", color: Palette.blue, group: "Body & nutrition") { AnyView(IBWTool()) },
        ToolItem(title: "BMR", icon: "bed.double.fill", color: Palette.purple, group: "Body & nutrition") { AnyView(BMRTool()) },
        ToolItem(title: "TDEE", icon: "bolt.fill", color: Palette.yellow, group: "Body & nutrition") { AnyView(TDEETool()) },
        ToolItem(title: "Protein", icon: "fork.knife", color: Palette.red, group: "Body & nutrition") { AnyView(ProteinTool()) },
        ToolItem(title: "Macros", icon: "chart.pie.fill", color: Palette.blue, group: "Body & nutrition") { AnyView(MacroTool()) },
        ToolItem(title: "Bulking", icon: "arrow.up.right", color: Palette.green, group: "Body & nutrition") { AnyView(BulkTool()) },
        ToolItem(title: "Recomp", icon: "arrow.triangle.2.circlepath", color: Palette.purple, group: "Body & nutrition") { AnyView(RecompTool()) },
    ]
}

/// Grid of calculators (Profile → Tools).
struct ToolsGrid: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ForEach(["Strength", "Body & nutrition"], id: \.self) { group in
                ...
                LazyVGrid(columns: [GridItem(.flexible(), spacing: 12), GridItem(.flexible(), spacing: 12), GridItem(.flexible(), spacing: 12)],
                          spacing: 12) {
                    ForEach(ToolCatalog.all.filter { $0.group == group }) { t in
                        NavigationLink {
                            t.make()
                            ...
                            VStack(spacing: 10) {
                                IconTile(icon: t.icon, color: t.color, size: 46)
                                Text(t.title).font(.caption.weight(.semibold)).lineLimit(1).minimumScaleFactor(0.8)
                            }
                            .frame(maxWidth: .infinity).padding(.vertical, 16)
                            .background(Palette.card, in: RoundedRectangle(cornerRadius: 20, style: .continuous))
                            .overlay(RoundedRectangle(cornerRadius: 20, style: .continuous).stroke(Palette.stroke))
                        }
                        .buttonStyle(.plain)
                    }
                }
...
struct StrengthLevelTool: View {
    ...
    var body: some View {
        ...
        Form {
            ...
            if let r = result {
                let rank = Rank.from(points: r.points)
                Section {
                    HStack(spacing: 12) {
                        RankBadge(level: rank.level, tier: rank.tier, size: 44)
                        Text(rank.name).font(.title2.bold()).foregroundStyle(Theme.color(rank.level))
                        Spacer()
                        Text("\(Int(ScoreScale.display(r.points).rounded())) / 100").monospacedDigit()
                    }
...
struct OneRMTool: View {
    ...
    var body: some View {
        ...
        let avg = results.map { $0.1 }.reduce(0, +) / Double(results.count)
        Form {
            Section {
